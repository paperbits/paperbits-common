...
export class NavbarModelBinder implements IModelBinder {
    ...
    public async nodeToModel(node: ContentConfig): Promise<NavbarModel> { //node = INavbar
    ...
    private async navigationItemToNavbarItemModel(navItem: INavigationItem, currentUrl:string): Promise<NavbarItemModel> {
    ...
    public getConfig(model: NavbarModel): ContentConfig {
        let navbarConfig: ContentConfig = {
            kind: "block",
            type: "navbar",
